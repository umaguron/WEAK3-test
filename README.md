...
# 実行環境
* __linux__ か __mac__
* WEAK3ではワークステーションとローカル(各自のマシン)の両方に同じ環境を構築することを想定
  * ローカル環境でも入力ファイルの準備や結果の可視化ができたほうが便利なので
* ローカル環境としてはmacを推奨。
  * 開発者はmacのターミナルで動作確認している
* windowsユーザーはWSLなどでlinux環境を用意する
  * pythonプログラムを動かすだけならcmdやpowershellでも問題ないが、いくつかのシェルスクリプトが使えない
...
4. 別環境(たとえばWS)で実行するには、作成されたinput.iniだけでなく、以下も必要であることに注意
   ...
   * 比抵抗構造データ
     ...
     これらをプロジェクトルート基準で同じ階層に配置する。プロジェクトルートにて、ファイル"util"に定義したコマンドを使うのが便利。utilの中に以下を書いておく必要がある。
     * ワークステーションのIPアドレス
     * ワークステーションのプロジェクトディレクトリのパス(~を含めず、~/以降を書く)
     * ローカル環境のプロジェクトディレクトリの絶対パス
     ...
     # ローカル --> ワークステーション
     send (コピーしたいファイル/ディレクトリの、プロジェクトルート基準の相対パス)
     
     # ワークステーション --> ローカル
     get (コピーしたいファイル/ディレクトリの、プロジェクトルート基準の相対パス)
