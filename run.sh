...
f=tempini/${suf}_${1////_}
    problemname=`python configUtil/parseConfig.py $f toughInput problemname`
    TID=`python configUtil/parseConfig.py $f configuration TOUGH_INPUT_DIR 2>/dev/null`
    if [ -z $TID ]; then
    settingIni=`python configUtil/parseConfig.py $f configuration configIni 2>/dev/null` 
    TID=`python configUtil/parseConfig.py $settingIni toughConfig TOUGH_INPUT_DIR  2>/dev/null`
    fi
    if [ -z $TID ]; then
    ...
    meshtype=`python configUtil/parseConfigMeshT.py $f`
...
echo ---- MAKE GRID
    if [ $meshtype == REGULAR ]; then python makeGrid.py $f -f
    elif [ $meshtype == A_VORO ]; then python makeGridAmeshVoro.py $f -f
    fi
echo ---- MAKE T2DATA 
    python tough3exec_ws.py $f -f
echo ---- EXEC 
    start=`date +%s`
    python run.py $f #-p 8
    #python run.py $f 
